...
// 常數 / Meta
// =============================================================================
const STAGE_ORDER = ['atrium', 'ventricle', 'avnode', 'treatment'];

const STAGE_META = {
  atrium: { title: '心房', subtitle: 'Atrium', variant: 'happy', tint: COLORS.coralSoft, ring: COLORS.coral },
  ventricle: { title: '心室', subtitle: 'Ventricle', variant: 'strong', tint: '#FFE9DC', ring: '#FF8A5B' },
  avnode: { title: '房室結', subtitle: 'AV Node', variant: 'link', tint: COLORS.mintSoft, ring: COLORS.mint },
  treatment: { title: '治療處置', subtitle: '危急心電圖判斷', variant: 'alert', tint: '#FFE1E7', ring: COLORS.coralDark },
};

const NEEDED_CORRECT = 3;
const FINAL_WRONG_PENALTY_MS = 20 * 1000;
...
// =============================================================================
// 簡易 store（React hooks 版本，僅供本次預覽使用）
// =============================================================================
function useGameStoreLite() {
  ...
  const actions = useMemo(
    () => ({
      setPlayerInfo: (name, unit) => setState((s) => ({ ...s, player: { name, unit } })),
      goToStart: () => setState((s) => ({ ...s, gamePhase: 'start' })),
      goToInfo: () => setState((s) => ({ ...s, gamePhase: 'info', player: { name: '', unit: '' } })),
      startGame: () =>
        setState((s) => {
      ...
      enterStage: (stageId, questionId) =>
        setState((s) => ({ ...s, gamePhase: 'stage_quiz', activeStageId: stageId, currentQuestionId: questionId })),
      setCurrentQuestion: (questionId) => setState((s) => ({ ...s, currentQuestionId: questionId })),
      answerStageQuestion: (isCorrect) =>
        setState((s) => {
          const stageId = s.activeStageId;
          if (!stageId) return s;
          ...
          if (isCorrect) {
            ...
            if (stage.correctCount >= NEEDED_CORRECT && !stage.completed) {
              ...
              const caseObj = FINAL_CASES.find((c) => c.id === s.selectedCaseId);
              stage.clue = caseObj ? caseObj.clues[stageId] : null;
            }
          } else {
            stage.wrongCount += 1;
          }
          return { ...s, stages: { ...s.stages, [stageId]: stage } };
        }),
      backToStageSelect: () => setState((s) => ({ ...s, gamePhase: 'stage_select', activeStageId: null, currentQuestionId: null })),
      enterFinalQuiz: () => setState((s) => ({ ...s, gamePhase: 'final_quiz' })),
      penalizeFinalWrong: () =>
        setState((s) => ({ ...s, penaltyMs: s.penaltyMs + FINAL_WRONG_PENALTY_MS, finalWrongAttempts: s.finalWrongAttempts + 1 })),
      finishGame: () =>
        setState((s) => {
          const endTime = Date.now();
          const totalMs = Math.max(0, endTime - s.startTime) + s.penaltyMs;
          const entry = {
            name: s.player.name?.trim() || '匿名實習生',
            unit: s.player.unit?.trim() || '-',
            timeMs: totalMs,
            date: new Date().toLocaleString('zh-TW', { hour12: false }),
          };
          const leaderboard = [...s.leaderboard, entry].sort((a, b) => a.timeMs - b.timeMs);
          return { ...s, gamePhase: 'leaderboard', endTime, leaderboard };
        }),
      restartRun: () => setState((s) => ({ ...s, gamePhase: 'start' })),
    }),
    []
  );
...
function Timer() {
  const [state] = useGame();
  const elapsed = useElapsedTime(state.startTime, state.penaltyMs, state.endTime);
  return (
    <div
      ...
      >
        ...
        <div className="space-y-4 font-body">
          ...
          <PillButton
            disabled={!canSubmit}
            onClick={() => {
              actions.setPlayerInfo(name, unit);
              actions.goToStart();
            }}
  ...
  return (
    <div className="min-h-[70vh] flex items-center justify-center">
      <div className="w-full max-w-lg rounded-[32px] shadow-xl p-8 border-4" style={{ background: '#fff', borderColor: COLORS.mintSoft }}>
        ...
        <PillButton onClick={actions.startGame} className="w-full py-3.5 text-base">
          開始遊戲・開始計時 💗
        </PillButton>
      </div>
    </div>
...
function StageCard({ stageId }) {
  ...
  const handleEnter = () => {
    ...
    actions.enterStage(stageId, q.id);
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-1.5">
        {Array.from({ length: NEEDED_CORRECT }).map((_, i) => (
        ...
        <span className="text-xs font-body ml-1" style={{ color: '#9A8AA3' }}>
          {Math.min(stage.correctCount, NEEDED_CORRECT)} / {NEEDED_CORRECT}
        </span>
      </div>
...
function StageSelectScreen() {
  const [state, actions] = useGame();
  const completedCount = STAGE_ORDER.filter((id) => state.stages[id].completed).length;
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="rounded-[28px] p-5 flex flex-col sm:flex-row items-center justify-between gap-3" style={{ background: COLORS.ink, color: '#fff' }}>
        ...
        <PillButton onClick={actions.enterFinalQuiz} className="px-6 py-2.5 whitespace-nowrap">
          🎯 前往最終診斷挑戰
        </PillButton>
      </div>
    </div>
...
function StageQuizScreen() {
  const [state, actions] = useGame();
  const { activeStageId, currentQuestionId } = state;
  const stage = state.stages[activeStageId];
  ...
  const handleSelect = (idx) => {
    ...
    actions.answerStageQuestion(isCorrect);
  };
  ...
  const handleNext = () => {
    ...
    setFeedback(null);
    if (stage.completed) {
      actions.backToStageSelect();
      return;
    }
    const nextQ = pickRandomQuestion(activeStageId, question.id);
    actions.setCurrentQuestion(nextQ.id);
  };
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <div className="rounded-[28px] shadow-md border-4 p-6" style={{ background: '#fff', borderColor: meta.tint }}>
        ...
        <div className="mt-5 flex justify-between items-center">
          <button onClick={actions.backToStageSelect} className="text-sm font-body hover:opacity-70 transition" style={{ color: '#B0A3B8' }}>
            ← 返回關卡選擇
          </button>
          {feedback && (
...
function FinalQuizScreen() {
  ...
  const optionCases = useMemo(
    () => state.finalOptionIds.map((id) => FINAL_CASES.find((c) => c.id === id)).filter(Boolean),
    [state.finalOptionIds]
  );

  const handleSelect = (caseId) => {
    if (feedback === 'correct') return;
    setSelected(caseId);
    if (caseId === state.selectedCaseId) {
      setFeedback('correct');
    } else {
      setFeedback('wrong');
      actions.penalizeFinalWrong();
    }
  };
      ...
      <div className="rounded-[28px] shadow-md border-4 p-6" style={{ background: '#fff', borderColor: COLORS.coralSoft }}>
        ...
        {feedback === 'correct' && (
          <div className="mt-4 rounded-2xl px-4 py-3 text-sm font-body font-semibold" style={{ background: COLORS.mintSoft, color: COLORS.mintDark }}>
            ✅ 診斷正確！
            <div className="mt-2">
              <PillButton onClick={actions.finishGame} tone="mint" className="px-4 py-1.5 text-xs">
                停止計時・查看成績
              </PillButton>
            </div>
          </div>
        ...
        <div className="mt-5 flex items-center justify-between text-xs font-body" style={{ color: '#B0A3B8' }}>
          <button onClick={actions.backToStageSelect} className="hover:opacity-70 transition">
            ← 返回關卡選擇（繼續收集線索）
          </button>
          {state.finalWrongAttempts > 0 && <span>已答錯 {state.finalWrongAttempts} 次（+{state.finalWrongAttempts * 20}s）</span>}
        </div>
      </div>
    </div>
  );
...
function LeaderboardScreen() {
  const [state, actions] = useGame();
  const { leaderboard, player, endTime, startTime, penaltyMs } = state;
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <p className="text-xs text-center font-body mb-4" style={{ color: '#B0A3B8' }}>
        ※ 此為 Artifact 預覽版本，排行榜僅存在本次瀏覽階段（記憶體），重新整理頁面會清空；正式版可用 zustand + persist 版本寫入 localStorage。
      </p>

      <div className="flex gap-3 justify-center">
        <PillButton onClick={actions.restartRun} className="px-5 py-2.5 text-sm">
          🔄 再次挑戰
        </PillButton>
        <PillButton onClick={actions.goToInfo} tone="dark" className="px-5 py-2.5 text-sm">
          更換使用者
        </PillButton>
      </div>
    </div>
...
// 主元件
// =============================================================================
export default function ECGGamePreview() {
  const gameState = useGameStoreLite();
  ...
  return (
    <GameContext.Provider value={gameState}>
      ...
      >
        {state.gamePhase === 'info' && <InfoScreen />}
        {state.gamePhase === 'start' && <StartScreen />}
        {state.gamePhase === 'stage_select' && <StageSelectScreen />}
        {state.gamePhase === 'stage_quiz' && <StageQuizScreen />}
        {state.gamePhase === 'final_quiz' && <FinalQuizScreen />}
        {state.gamePhase === 'leaderboard' && <LeaderboardScreen />}
      </div>
